...
const PillarsofIslam = () => {
  const sections = [
    ...
    },
    {
      ...
      description:
        "Zakah purifies wealth by redistributing it to those less fortunate. It’s not just charity — it’s a duty for every eligible Muslim. Through Zakah, the gap between the rich and poor is reduced, and society becomes more compassionate and balanced. Recipients may include the poor, the needy, and those in debt.",
  ...
  const itemVariants = (isEven) => ({
    hidden: {
      opacity: 0,
      x: isEven ? 100 : -100, // Slide from right if even (row-reverse), left if odd
    },
    visible: {
      opacity: 1,
      x: 0,
    ...
    <Container>
      ...
      >
        {sections.map((item, index) => {
          const isEven = index % 2 !== 0; // Your original $isEven logic
          return (
            <Section
              as={motion.div}
              key={index}
